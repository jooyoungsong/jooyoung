...
public class SimpleBoardDao {
	...
	public int getMaxNum()
	{
		...
		try {
			...
			if(rs.next())
			{
				max=rs.getInt("max");  //max=rs.getInt(1); 퀀텀에서 1이 들어가는 건 index 넣어도 되는 거라!!(컬럼의 열)
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public boolean getCheckPass(String num,String pass)
	{
		...
		try {
			...
			if(rs.next())
			{
				...
				if(rs.getInt(1)==1) //비번이 틀릴 경우 0임 //count(*)가 getint(1)로 표현된 것
					check=true;
	...
	//페이징 처리_1. 전체 갯수 반환 (이거 항상 복사해도 가능한 페이징 처리)
	public int getTotalcount()
	{
		...
		try {
			...
			if(rs.next())
			{
				total=rs.getInt(1);  //count만 구할 거니깐 1번열인 '1'을 작성
			}
